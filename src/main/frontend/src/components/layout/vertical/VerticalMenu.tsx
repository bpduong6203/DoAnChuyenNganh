  ...
  return (
    ...
    >
      ...
      <Menu
        ...
        renderExpandedMenuItemIcon={{ icon: <RiCircleLine /> }}
        menuSectionStyles={menuSectionStyles(theme)}
      >
        <MenuItem icon={<LuLayoutDashboard className='w-4 h-4' href='/' />}>Analytics</MenuItem>
        <SubMenu
          label='Dashboards'
          icon={<i className='ri-home-smile-line' />}
          suffix={<Chip label={adminRoutes.length} size='small' color='error' />}
        >
          {adminRoutes.map(route => (
            <MenuItem key={route.href} href={route.href} icon={route.Icon}>
              {route.label}
            </MenuItem>
          ))}
        </SubMenu>
